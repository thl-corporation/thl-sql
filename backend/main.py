...
def get_profile_settings():
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT app_name, logo_data FROM app_profile_settings WHERE id = 1")
        row = cur.fetchone()
        if row:
            return {"app_name": row[0] or APP_DEFAULT_NAME, "logo_data": row[1]}
        return {"app_name": APP_DEFAULT_NAME, "logo_data": None}
    finally:
        cur.close()
        conn.close()
...
def save_profile_settings(app_name: str, logo_data: str | None):
    ...
    try:
        cur.execute("""
            INSERT INTO app_profile_settings (id, app_name, logo_data, updated_at)
            VALUES (1, %s, %s, NOW())
...
def update_profile_password(req: PasswordUpdateRequest, username: str = Depends(get_current_username), csrf_ok: bool = Depends(require_csrf)):
    global ADMIN_PASSWORD
    if not secrets.compare_digest(req.current_password, ADMIN_PASSWORD):
    ...
    session_store.clear()
    return {"status": "success", "message": "Contraseña actualizada. Debes iniciar sesión nuevamente."}
